...
# Section for exploring the best fitting model(s)
...
newdat<-newdat%>%dplyr::mutate(by_cat = case_when(BIRTH_YR < quantile(newdat$BIRTH_YR, c(0.33,0.66))[[1]] ~ 0,
                                                  ...
                                                  TRUE ~1))#
dat<-read.csv("N:/durable/users/norarba/FLA2_rew/df_alldat_short.csv")

dat$edu_mf<-dat$hig_edu_mor_or_far
str(dat$edu_mf)
dat$edu_mf<-as.integer(as.logical(dat$edu_mf))
dat$edu_mf<-as.character(dat$edu_mf)
str(dat$edu_mf)

variable.names(newdat)
dat_edu<-dat%>%mutate(IID=paste(preg_id,BARN_NR,sep=""))
dat_edu<-dat_edu%>%select(IID,edu_mf)
dat_edu$IID<-as.integer(dat_edu$IID)
newdat2<-left_join(newdat,dat_edu,by="IID")

prepareMplusData(newdat2, "./mplus_ses/data/data_for_mplus_step3.dat")
...
filepath2 <- "./mplus_ses/scripts/step3"
...
mplusOutput2 <- readModels("./mplus_ses/output/step3", recursive=TRUE)#


classcounts_3step_dep<- cbind(data.frame(mplusOutput2$..mplus_ses.output.step3.lpa_5c_step3_dep.out$class_counts$mostLikely),
                              data.frame(mplusOutput2$..mplus_ses.output.step3.lpa_5c_step3_dep.out$class_counts$avgProbs.mostLikely)) %>% 
  rename("prob_Class1"=X1,
         "prob_Class2"=X2,
         "prob_Class3"=X3,
         "prob_Class4"=X4,
...
classcounts_3step_anx<- cbind(data.frame(mplusOutput2$..mplus_ses.output.step3.lpa_5c_step3_anx.out$class_counts$mostLikely),
                              data.frame(mplusOutput2$..mplus_ses.output.step3.lpa_5c_step3_anx.out$class_counts$avgProbs.mostLikely)) %>% 
  rename("prob_Class1"=X1,
         "prob_Class2"=X2,
         "prob_Class3"=X3,
         "prob_Class4"=X4,
...
classcounts_3step_neurot<- cbind(data.frame(mplusOutput2$..mplus_ses.output.step3.lpa_5c_step3_neurot.out$class_counts$mostLikely),
                                 data.frame(mplusOutput2$..mplus_ses.output.step3.lpa_5c_step3_neurot.out$class_counts$avgProbs.mostLikely)) %>% 
  rename("prob_Class1"=X1,
         "prob_Class2"=X2,
         "prob_Class3"=X3,
         "prob_Class4"=X4,
...
classcounts_3step_bd<- cbind(data.frame(mplusOutput2$..mplus_ses.output.step3.lpa_5c_step3_bd.out$class_counts$mostLikely),
                             data.frame(mplusOutput2$..mplus_ses.output.step3.lpa_5c_step3_bd.out$class_counts$avgProbs.mostLikely)) %>% 
  rename("prob_Class1"=X1,
         "prob_Class2"=X2,
         "prob_Class3"=X3,
         "prob_Class4"=X4,
...
classcounts_3step_anyemod<- cbind(data.frame(mplusOutput2$..mplus_ses.output.step3.lpa_5c_step3_anyemod.out$class_counts$mostLikely),
                                  data.frame(mplusOutput2$..mplus_ses.output.step3.lpa_5c_step3_anyemod.out$class_counts$avgProbs.mostLikely)) %>% 
  rename("prob_Class1"=X1,
         "prob_Class2"=X2,
         "prob_Class3"=X3,
         "prob_Class4"=X4,
...
dx_res <-map(mplusOutput2, function(x){
  ...
  purrr::reduce(bind_rows)
dx_res
dx_res2<-dx_res[11:20, ]
library(arsenal)
write2word(dx_res2, "N:/durable/users/norarba/FLA2_rew/mplus_ses/output/step3/anyemo_OR_ses.doc")
write.table(dx_res2, file = "N:/durable/users/norarba/FLA2_rew/mplus_ses/output/step3/anyemo_OR_ses.txt", sep = ",", quote = FALSE, row.names = F)
...
cov_res2
  ###NBNB suddenly several edu_mf for adj anx else they look normal. 
str(cov_res)
cov_res2 <-subset(cov_res, covariate!="    SEX" & covariate!="    BY_CAT" & covariate!="    EDU_MF")#note spacing 
cov_res2

write.table(cov_res2, file = "N:/durable/users/norarba/FLA2_rew/mplus_ses/output/step3/prs_OR_ref2_ses.txt", sep = ",", quote = FALSE, row.names = F)
...
str(cov_res)
cov_res2 <-subset(cov_res, covariate!="    SEX" & covariate!="    BY_CAT"& covariate!="    EDU_MF")#note spacing making confusion
cov_res2
write.table(cov_res2, file = "N:/durable/users/norarba/FLA2_rew/mplus_ses/output/step3/prs_OR_ref1_ses.txt", sep = ",", quote = FALSE, row.names = F)
...
str(cov_res)
cov_res2 <-subset(cov_res, covariate!="    SEX" & covariate!="    BY_CAT"& covariate!="    EDU_MF")#note spacing is the thin
cov_res2

write.table(cov_res2, file = "N:/durable/users/norarba/FLA2_rew/mplus_ses/output/step3/prs_OR_ref3_ses.txt", sep = ",", quote = FALSE, row.names = F)
...
write.table(cov_res2, file = "N:/durable/users/norarba/FLA2_rew/mplus_ses/output/step3/prs_OR_ref4_ses.txt", sep = ",", quote = FALSE, row.names = F)

####dubble check numbers in Mplus output files. 
